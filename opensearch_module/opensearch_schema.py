...
class IndexingSchema(BaseModel):
    profileId:str = Field(..., min_length=1, description="Unique pofileId")
    profileText:str = Field(..., min_length=1, description="profile text")
    source_domain:str = Field(..., min_length=1)
    lunaIds: conlist(item_type=str, min_items=1, description="luna_ids: list[str]")
    svc_mgmt_nums: conlist(item_type=str, min_items=1, description="svc_mgmt_nums: list[str]")
